#!/usr/bin/env python3
"""
Cisco ISE - Create Internal Users via ERS API from CSV (multi-node, multi-group)
...
- If the CSV has blank identity_groups values, the script can ask for one shared group list
...
def create_user_on_base(
    ...
    valid_group_ids, missing_group_names, group_error, group_http_code = (
        resolve_existing_identity_group_ids(session, base, group_names)
    )
    if group_error:
        return False, group_error, group_http_code
    ...
    if r_post.status_code in (200, 201):
        return True, (
            ...
            f"groups='{valid_group_names_text}' "
...
def parse_group_names(value: str) -> list[str]:
    """Parse semicolon-separated group names and remove duplicates."""
    groups: list[str] = []
    seen: set[str] = set()
...
def resolve_existing_identity_group_ids(
    ...
    if missing_group_names:
        log_event(
            "WARNING: Skipping non-existing Identity Group(s): "
...
def create_user_across_hosts(
    ...
    log_event(
        ...
        f"groups='{';'.join(group_names)}' description='{description}'"
    )
